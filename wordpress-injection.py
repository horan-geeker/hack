...
def get_posts(api_base):
    respone = urlopen(api_base + 'wp/v2/posts')
...
if __name__ == '__main__':
    ...
    for site in sites:
        try:
            ...
            print('* Updating post {0}'.format(posts[0]['id']))
            # with open(sys.argv[3], 'r') as content:
            with open('signature', 'r') as content:
                new_content = content.readlines()

            update_post(api_url, posts[0]['id'], ''.join(new_content))
